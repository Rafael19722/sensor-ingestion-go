...
func AuthMiddleware() gin.HandlerFunc {
    ...
    authHeader := c.GetHeader("Authorization")
		if authHeader == "" {
			c.JSON(http.StatusUnauthorized, gin.H{"error": "Authorization Header absent"})
			c.Abort()
			return
		}
    ...
    if !ok {
      c.JSON(http.StatusUnauthorized, gin.H{"error": "Failed to read data (claims) from token"})
      c.Abort()
      return
    }
    ...
    if !exists {
      c.JSON(http.StatusUnauthorized, gin.H{"error": "tenant_id is missing from token claims"})
      c.Abort()
      return
    }
